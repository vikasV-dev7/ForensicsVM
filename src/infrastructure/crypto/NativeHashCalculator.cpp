...
std::expected<std::string, fvm::contracts::HashError> NativeHashCalculator::calculateSha256(const std::string& data) {
#ifdef _WIN32
    BCRYPT_ALG_HANDLE hAlg = NULL;
    BCRYPT_HASH_HANDLE hHash = NULL;
    DWORD cbData = 0, cbHash = 0, cbHashObject = 0;
    ...
    if (!data.empty()) {
        if (!BCRYPT_SUCCESS(BCryptHashData(hHash, (PBYTE)data.data(), static_cast<ULONG>(data.size()), 0))) {
            BCryptDestroyHash(hHash);
            BCryptCloseAlgorithmProvider(hAlg, 0);
            return std::unexpected(fvm::contracts::HashError::CryptoError);
        }
